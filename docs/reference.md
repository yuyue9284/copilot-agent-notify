# Configuration and developer reference

Start with the [gadget quick start](../README.md) for installation and everyday
use. This guide covers detailed gadget behavior, optional notification hooks,
terminal integrations, diagnostics, and development.

All commands below assume the **repository root** as the working directory.

## Optional notification plugin overview
...
Use the **Appearance** gear in the header (in either layout) to choose:

- **Auto** (default): native Desktop Acrylic in supported local sessions on Windows
  11 build 22621 or later; plain translucency in remote sessions or when Acrylic is unsupported.
- **Acrylic**: explicitly request the native material, even remotely. Windows may
  render it solid; an unsupported API falls back to plain translucency.
...
fully opaque composition while retaining the selected mode. Plain translucency
blends text and the standard title bar too; Acrylic keeps its foreground content
...
**Desktop notifications** in the same menu are enabled by default. While the
gadget is running, it sends a Windows notification when an observed session
first enters **Needs input** or transitions from working/waiting to **Done**.
Sessions already waiting or done when a collector first connects do not produce
retroactive notifications. Repeated snapshots and temporary collector failures
...
`{"compact":true,"appearance":"auto","opacity":90,"font_family":"Segoe UI","font_size":13,"notifications":true}`.
Existing files default to Segoe UI 13 and Auto/90; no manual JSON editing is
needed. Existing files also default desktop notifications to enabled. If saving
fails, controls revert and the existing error panel reports
the failure.
...
These system preferences override the selected appearance, without overwriting
the saved choice. The gadget observes appearance/session changes without changing system settings.
...
- Windows Python 3.9+, .NET Framework 4.8 with WPF assemblies,
  ...
  tools. Windows Python runs the shared discovery and activity-classification
  code; C# owns the application host and window.
...
runner covers configuration, worker lifecycle, and window-pipe behavior:
allowlists and discovery, stale/failed source recovery,
...
To suppress desktop notifications and terminal bells generated by the hooks,
create `~/.copilot/copilot-agent-notify.json` (or place it under `COPILOT_HOME`)
with:
...
This is useful when the Windows gadget is providing alerts. It does not disable
the gadget, Zellij pane/tab activity icons, or Windows Terminal progress. Missing
configuration or `"hook_alerts":true` enables hook alerts. Invalid values are
reported. `COPILOT_NOTIFY_HOOK_ALERTS=0` or `1` remains available as a temporary
environment override and takes precedence over the file.
...
- `scripts/`: shared session collectors, notification delivery, and Zellij activity coordination.
- `windows-helper/gadget/`: native WPF session window and collector host.
- `windows-helper/`: gadget build/install scripts and native notification helper.
